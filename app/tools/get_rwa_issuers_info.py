...
# Issuer list changes infrequently — cache per distinct `limit` value to
# avoid re-fetching on every call while a session repeatedly asks about it.
...
async def get_rwa_issuers_info(limit: int = 50) -> Dict[str, Any]:
    ...
    try:
        ...
        return await shared_cache.get_or_set(cache_key, _fetch, ttl_seconds=_CACHE_TTL_SECONDS)
