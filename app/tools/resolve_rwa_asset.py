...
async def _lookup_parent_rwa_by_token_symbol(token_symbol: str) -> Optional[Dict[str, Any]]:
    """Resolve a token symbol like PAXG or XAUt back to its parent RWA asset (e.g. GOLD)."""
    start = 1

    while True:
        issuer_page = await cmc_client.get_rwa_issuers_list(limit=250, start=start)
        ...
        for issuer in issuers:
            ...
            issuer_details = await cmc_client.get_rwa_issuer(issuer_id=issuer_id, limit=250)
            for token in issuer_details.get("tokens", []):
                ...
                parent_data = await cmc_client.get_rwa_quotes(rwa_id=str(rwa_id))
...
async def resolve_rwa_asset(symbol: str) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        try:
            data = await cmc_client.get_rwa_quotes(symbol=clean_symbol)
        except Exception:
            data = {"rwa_assets": []}
        ...
        resolved_parent = await _lookup_parent_rwa_by_token_symbol(clean_symbol)
        if resolved_parent:
